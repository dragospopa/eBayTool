...
if ($err) {
  ...
  foreach($resp->itemSummaries as $item) {
    ...
    $sql .=  "INSERT INTO auctions (itemID, productName, highestBid, currency, thumbnailPhotoURL, sellerUsername, sellerFeedbackPercentage, itemCondition, bidCount, auctionStartTime, auctionEndTime, buyingOptions) values (\"$itemId\", \"$productName\", $highestBid, \"$currency\",\"$thumbnailPhotoURL\", \"$sellerUsername\", $sellerFeedbackPercentage, \"$itemCondition\", $bidCount, \"$auctionStartTime\", \"$auctionEndTime\", \"$buyingOptions\"); ";
    print_r($sql);
